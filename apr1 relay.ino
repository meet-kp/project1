...
void socketIOEvent(const socketIOmessageType_t& type, uint8_t * payload, const size_t& length)
{
  switch (type)
  {
    ...
    case sIOtype_EVENT:
      ...
      // Check if the message contains the string "ping" and "status=1"
      if (strstr((char*)payload, "ping") != NULL && strstr((char*)payload, "status=1") != NULL) {
        Serial.println("Received ping with status=1. Triggering relay.");
        digitalWrite(relayPin, HIGH); // give 5V to the relay
