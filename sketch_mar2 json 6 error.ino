...
void socketIOEvent(const socketIOmessageType_t& type, uint8_t * payload, const size_t& length)
{
  switch (type)
  {
    ...
    case sIOtype_EVENT:
      ...
      // Check if the event name is "unlock_relay"
      if (strcmp(doc[0], "unlock_relay") == 0) {
        // Get the relay number to unlock
        int relayNumber = doc[1]["relay_number"];

        // Unlock the relay
        unlockRelay(relayNumber);
      }
...
void unlockRelay(int relayNumber)
{
  // Implement the code to unlock the specified relay
  // For example, if you have connected the relay to pin D1 on the ESP8266
  digitalWrite(D1, LOW);
  delay(1000);
